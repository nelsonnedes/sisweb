...
- **Commit + deploy (sessão seguinte, autoridade devops):** `node tools/inject-cachebusters.mjs` (8 HTMLs), `npm run build:hosting` (478 arquivos), commit `98f3c0d` "fix(print-mobile): janela apos dados + Voltar + trigger robusto (about:blank)" (15 arquivos), `git push main` OK, `firebase deploy --only hosting` "Deploy complete!" em `https://sisweb-7ce82.web.app`; `hosting-dist/commerce-pdf-share.js` e `hosting-dist/modules/reports/imprimir-romaneio.js` verificados com `sisweb-print-back`/`ensureRomaneioPrintAux`.
