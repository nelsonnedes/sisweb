...
- **FIX pós-deploy — sobreposição dos filtros nos offcanvas (2026-08-19, `financas-mobile-v2`):** usuário reportou campos se sobrepondo no offcanvas mobile de Receber/Pagar. Causa raiz: o `<style>` do `<head>` de `financas.html` (linhas ~389-428) define `.filters-row{display:grid; repeat(auto-fit,minmax(200px,1fr))}` e, em `@media (max-width:1200px)`, `.filters-row-receber`/`.filters-row-pagar` com `grid-template-columns:repeat(3,minmax(200px,1fr))` + `grid-template-areas` (especificidade 0-2-0) — vencia o override mobile `1fr` (0-1-0) do bloco de filtros no fim do body. Fix no `@media (max-width:768px)`: `.filters-row, .filters-row.filters-row-receber, .filters-row.filters-row-pagar{ grid-template-columns:1fr !important; grid-template-areas:none !important; grid-template-rows:none !important; gap:12px }` **E** reset de posicionamento nos filhos: `.filters-row.filters-row-receber .form-group, .filters-row.filters-row-pagar .form-group{ grid-area:auto !important; grid-row:auto !important; grid-column:auto !important }`. O segundo é OBRIGATÓRIO: os `.fg-*` do head carregam `grid-area:status/cliente/...` que, com areas:none, criam trilhas implícitas e colapsam todos os campos na mesma célula (diagnóstico: `getComputedStyle().gridTemplateColumns` mostrava `11px 0px 192px` mesmo com `1fr !important` inline — o shorthand `grid-template` computado confirmava 1 coluna; eram trilhas implícitas dos filhos). `sw.js` bumpado para `2026-08-19-financas-mobile-v2` + os 5 testes que assertam APP_VERSION atualizados; teste `estoque-pwa-impressao.test.mjs` (linha 33) atualizado para o novo texto do override. Suíte **475 pass / 0 fail / 1 skip**; deploy revalidado em produção (Receber e Pagar `gridCols=227px`, `overlaps=[]`, docSW==clientW; desktop reverte para auto-fit sem sobreposição). Screenshot: `C:\Users\Nelson\AppData\Local\Temp\opencode\financas-prod-filtros-fixed.png`.

## 14. Como manter este cérebro
