...
function insertBridgeImport(html) {
  ...
  const initImportMatch = html.match(/(import\s+.*from\s+['"].\/firebase-init\.js['"].*)\n/);
  if (initImportMatch) {
    const after = initImportMatch.index + initImportMatch[0].length;
    return html.slice(0, after) + `import './firebase-compat-bridge.js';  // compat shim\n` + html.slice(after);
  }
